...
class MediaViewerViewModelTest {
    ...
    @Test
    fun `uiState becomes Error when initial media is not found`() =
        runTest {
    ...
    @Test
    fun `Ready state lists all media types together`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
    ...
    @Test
    fun `items follow LibraryDisplayState ordering when present`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            fakeMediaRepository.addMedia(photoItem("p2"))
    ...
    @Test
    fun `items exclude entries missing from LibraryDisplayState`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            fakeMediaRepository.addMedia(photoItem("p2"))
            fakeMediaRepository.addMedia(photoItem("p3"))
    ...
    @Test
    fun `Ready state sets currentIndex to position of initial mediaId`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            fakeMediaRepository.addMedia(videoItem("v1"))
            fakeMediaRepository.addMedia(photoItem("p2"))
    ...
    @Test
    fun `onMediaChanged updates currentIndex`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            fakeMediaRepository.addMedia(videoItem("v1"))
            val viewModel = buildViewModel("p1")
    ...
    @Test
    fun `onMediaChanged loads saved playback state when target is a video`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            fakeMediaRepository.addMedia(videoItem("v1"))
    ...
    @Test
    fun `toggleToolbar hides toolbar when visible`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            val viewModel = buildViewModel("p1")
    ...
    @Test
    fun `setAbPoint A updates abStartMs when current is a video`() =
        runTest {
            fakeMediaRepository.addMedia(videoItem("v1"))
            val viewModel = buildViewModel("v1")
    ...
    @Test
    fun `setAbPoint is no-op when current is a photo`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            val viewModel = buildViewModel("p1")
    ...
    @Test
    fun `deleteCurrentMedia removes the item from the library`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            fakeMediaRepository.addMedia(photoItem("p2"))
            val viewModel = buildViewModel("p1")
    ...
    @Test
    fun `deleteCurrentMedia emits navigateBack when last item is deleted`() =
    ...
    @Test
    fun `saveCurrentState does not persist for photo items`() =
        runTest {
            fakeMediaRepository.addMedia(photoItem("p1"))
            val viewModel = buildViewModel("p1")
    ...
    private fun videoItem(id: String) =
        MediaItem(
            id = id,
    ...
    private fun playbackState(videoId: String) =
        io.github.kiyohitonara.biwa.domain.model.PlaybackState(
            videoId = videoId,
            positionMs = 0L,
            abStartMs = null,
            abEndMs = null,
