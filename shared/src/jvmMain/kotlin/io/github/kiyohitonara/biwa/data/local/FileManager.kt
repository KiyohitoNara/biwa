...
actual class FileManager : FileStorage {
    actual override suspend fun copyToInternalStorage(
        sourceUri: String,
        fileName: String,
    ): String = error("FileManager is not supported on JVM")

    actual override suspend fun deleteFromInternalStorage(filePath: String): Unit = error("FileManager is not supported on JVM")
}
