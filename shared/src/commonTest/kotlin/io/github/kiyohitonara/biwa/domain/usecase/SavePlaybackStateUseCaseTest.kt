...
class SavePlaybackStateUseCaseTest {
    ...
    @Test
    fun `execute persists state with clock timestamp`() =
        runTest {
            useCase.execute(
                videoId = "video-1",
                positionMs = 10_000L,
    ...
    @Test
    fun `execute preserves all fields`() =
        runTest {
            useCase.execute(
                videoId = "video-1",
                positionMs = 10_000L,
    ...
    @Test
    fun `execute preserves null ab points`() =
        runTest {
            useCase.execute(
                videoId = "video-1",
                positionMs = 0L,
    ...
    @Test
    fun `execute overwrites existing state`() =
        runTest {
