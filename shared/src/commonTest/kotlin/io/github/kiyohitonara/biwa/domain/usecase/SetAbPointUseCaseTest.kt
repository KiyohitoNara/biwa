...
class SetAbPointUseCaseTest {
    ...
    @Test
    fun `execute sets A point when no existing state`() =
        runTest {
    ...
    @Test
    fun `execute sets B point when no existing state`() =
        runTest {
    ...
    @Test
    fun `execute updates A point preserving existing B`() =
        runTest {
    ...
    @Test
    fun `execute updates B point preserving existing A`() =
        runTest {
    ...
    @Test
    fun `execute returns Success when A point only set`() =
        runTest {
    ...
    @Test
    fun `execute returns Success when B point only set`() =
        runTest {
    ...
    @Test
    fun `execute returns Success when B is greater than A`() =
        runTest {
    ...
    @Test
    fun `execute returns InvalidRange when B equals A`() =
        runTest {
            stored["video-1"] = playbackState().copy(abStartMs = 5_000L, abEndMs = null)
    ...
    @Test
    fun `execute returns InvalidRange when B is less than A`() =
        runTest {
    ...
    @Test
    fun `execute does not save state when range is invalid`() =
        runTest {
    ...
    @Test
    fun `execute uses defaults for position and speed when no prior state`() =
        runTest {
            useCase.execute("video-1", AbPoint.A, 5_000L)
    ...
    @Test
    fun `execute preserves existing position and speed`() =
        runTest {
