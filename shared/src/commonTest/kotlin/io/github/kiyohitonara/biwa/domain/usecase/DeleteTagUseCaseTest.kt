...
class DeleteTagUseCaseTest {
    ...
    @Test
    fun `execute removes tag from repository`() =
        runTest {
            createTag("t1", "Nature")
    ...
    @Test
    fun `execute does not affect other tags`() =
        runTest {
            createTag("t1", "Nature")
            createTag("t2", "Travel")
    ...
    @Test
    fun `execute removes media associations for deleted tag`() =
        runTest {
            createTag("t1", "Nature")
    ...
    @Test
    fun `execute is no-op when tag does not exist`() =
        runTest {
